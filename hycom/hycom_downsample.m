...
energy_rescale = false;  % No added noise
...
xratio = 3;  % Sample every so many pixels (total downsampling will be ratio^2)
yratio = 3;
...
figure();
subplot(211)
xplot = x; xplot(flow.mask) = NaN;
pcolor(reshape(xplot, ny, nx)); hold on
%scatter(sensor_locs(:, 1), sensor_locs(:, 2), 'k.')
...
subplot(212)
yplot = y;
yplot(ismember(sensor_idx, find(mask))) = NaN;
pcolor(reshape(yplot, ny_small, nx_small))
shading flat; caxis(flow.clim);
set(gca, 'Color', [100 100 100]/255);  % Set NaN (as background) to gray
set(gca,'xtick',[]); set(gca,'ytick',[])

colormap(flow.cmap)
set(gcf, 'Position', [100 500 2*nx 4*ny])

%% Construct dictionaries
disp('Building dictionaries')

sensor_idx = sensor_idx(~ismember(sensor_idx, find(mask))); % Restrict to non-land points
...
%s_sr = omp(D, y, D'*D, K);
%[x_sr, res_sr] = reconstruct(x, Train, full(s_sr).\omp_rescale, flow, false);

% Identify sparse coefficients
s_sr = sp_approx(y, D, 0, flow);
% Reconstruct from sparse representation
[x_sr, res_sr] = reconstruct(x, Train, full(s_sr), flow, energy_rescale);
K_sr = sum(abs(s_sr)>1e-6);

%% Least-squares POD (global)
s_pod = (double(U(sensor_idx, 1:r)))\y;  % Coefficients from least squares solution
...
for j=1:k
    ...
    if ~isempty(y)  % If kernel isn't just land
        ...
        [x_hat, ~] = reconstruct(x_j, library, s, local_flow, energy_rescale);
        x_local_sr(kernel_idx) = x_local_sr(kernel_idx) + x_hat;
        K_local_sr = [K_local_sr sum(abs(s)>1e-6)];  % Compute sparsity of solution
        
        % L2 POD
        r_window = floor(length(kernel_sensors)/2);
        s = (C*U(:, 1:r_window))\y;
        [x_hat, ~] = reconstruct(x_j, kernel*U(kernel_idx, 1:r_window), s, local_flow, energy_rescale);
...
save('../output/hycom/downsampled_examples.mat', 'x', 'sensor_idx', 'x_measure' ...
    , 's_sr', 'x_sr', 'res_sr', 'x_pod', 's_pod', 'res_pod', 'flow'...
    , 'x_local_sr', 'res_local_sr', 'x_local_pod', 'res_local_pod'...
    , 'K_sr', 'K_pod', 'K_local_sr', 'K_local_pod', 'k', 'ns')
